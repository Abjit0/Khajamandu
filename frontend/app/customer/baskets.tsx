...
export default function BasketsScreen() {
  ...
  // Reload cart when params change (new items added)
  useEffect(() => {
    if (params.data) {
      loadCart();
    }
  ...
  const loadCart = async () => {
    try {
      ...
      if (params.data) {
        ...
        // Check if there's stored cart data
        const storedCart = await AsyncStorage.getItem(CART_STORAGE_KEY);
        
        if (storedCart) {
          console.log('Loading from storage:', storedCart);
      ...
      <ScrollView style={styles.content}>
        {isLoading ? (
          <View style={styles.emptyState}>
            <Text style={styles.emptyTitle}>Loading your basket...</Text>
          </View>
        ) : cartItems.length === 0 ? (
          <View style={styles.emptyState}>
