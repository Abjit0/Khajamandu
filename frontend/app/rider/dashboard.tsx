...
import { 
  View, Text, StyleSheet, TouchableOpacity, ScrollView, Alert, ActivityIndicator, Switch
} from 'react-native';
...
export default function RiderDashboard() {
  ...
  const ensureRiderProfile = async () => {
    try {
      ...
      if (profileResponse.data.status === 'SUCCESS') return;
    } catch (error: any) {
  ...
  const confirmCODCollection = (order: any) => {
    Alert.alert('Cash on Delivery', `Collect Rs ${order.totalAmount} from customer and confirm delivery.`, [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Cash Collected',
        onPress: () => {
          Alert.alert('Confirm Delivery', 'Have you collected the cash and delivered the order?', [
            { text: 'No', style: 'cancel' },
            { text: 'Yes, Delivered', onPress: () => updateDeliveryStatus('delivered', true) }
          ]);
        }
      }
    ]);
          ...
          <View key={order._id} style={styles.orderCard}>
            ...
            <View style={styles.orderInfo}>
              <View style={styles.infoRow}>
                <View style={styles.infoIcon}><Ionicons name="restaurant" size={18} color={COLORS.primary} /></View>
              ...
              <View style={styles.infoRow}>
                <View style={styles.infoIcon}><Ionicons name="location" size={18} color={COLORS.error} /></View>
                <View style={styles.infoContent}>
              ...
              <View style={styles.infoRow}>
                <View style={styles.infoIcon}><Ionicons name="person" size={18} color={COLORS.blue} /></View>
                <View style={styles.infoContent}>
            ...
            <TouchableOpacity style={styles.acceptButton} onPress={() => acceptDelivery(order._id)}>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.activeDeliveryCard}>
          ...
          <View style={styles.activeOrderInfo}>
            ...
            <View style={styles.divider} />
            <View style={styles.infoRow}>
              <View style={styles.infoIcon}><Ionicons name="call" size={18} color={COLORS.success} /></View>
              <View style={styles.infoContent}>
            ...
            <View style={styles.divider} />
            <View style={styles.infoRow}>
              <View style={styles.infoIcon}><Ionicons name="card" size={18} color={isCOD ? COLORS.warning : COLORS.success} /></View>
              <View style={styles.infoContent}>
          ...
          <View style={styles.actionButtonsContainer}>
            {activeDelivery.status === 'assigned' && (
              <TouchableOpacity style={styles.statusButtonLarge} onPress={() => updateDeliveryStatus('picked_up')}>
                <Ionicons name="bag-check" size={22} color={COLORS.white} />
                <Text style={styles.statusButtonText}>Picked Up</Text>
              </TouchableOpacity>
            )}
            {activeDelivery.status === 'picked_up' && (
              <TouchableOpacity style={styles.statusButtonLarge} onPress={() => updateDeliveryStatus('on_the_way')}>
            ...
            {(activeDelivery.status === 'on_the_way' || activeDelivery.status === 'picked_up') && (
              <TouchableOpacity style={styles.completeButtonLarge} onPress={() => {
                if (isCOD) { confirmCODCollection(order); } else { updateDeliveryStatus('delivered'); }
              }}>
                <Ionicons name="checkmark-done" size={22} color={COLORS.white} />
                <Text style={styles.completeButtonText}>{isCOD ? 'Collect & Deliver' : 'Mark Delivered'}</Text>
              </TouchableOpacity>
            )}
          </View>
        </View>
    ...
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          <TouchableOpacity style={styles.profileButtonLeft} onPress={() => router.push('/rider/profile' as any)}>
            {profileImage ? (
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    alignItems: 'center', backgroundColor: COLORS.white, elevation: 4,
  ...
  profileButtonLeft: {
    ...
    backgroundColor: COLORS.primaryLight, borderRadius: 22, overflow: 'hidden'
  },
  ...
  featuredCard: {
    backgroundColor: COLORS.white, padding: 20, borderRadius: 20, elevation: 3,
  ...
  orderCard: {
    backgroundColor: COLORS.white, padding: 20, borderRadius: 16, marginBottom: 12,
    elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 4
  },
  orderCardHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 },
  orderBadge: { backgroundColor: COLORS.successLight, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8 },
  ...
  activeDeliveryCard: {
    backgroundColor: COLORS.white, padding: 20, borderRadius: 20, elevation: 4,
    shadowColor: COLORS.primary, shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.15, shadowRadius: 12, borderWidth: 2, borderColor: COLORS.primaryLight
  },
  ...
  progressBar: { height: 6, backgroundColor: COLORS.lightGray, borderRadius: 3, overflow: 'hidden', marginBottom: 8 },
  ...
  statusButtonLarge: {
    flexDirection: 'row', backgroundColor: COLORS.warning, padding: 16,
    borderRadius: 12, alignItems: 'center', justifyContent: 'center', gap: 8
  },
  statusButtonText: { color: COLORS.white, fontSize: 16, fontWeight: '700' },
  completeButtonLarge: {
    flexDirection: 'row', backgroundColor: COLORS.success, padding: 16,
    borderRadius: 12, alignItems: 'center', justifyContent: 'center', gap: 8
  },
